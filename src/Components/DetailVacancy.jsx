...
import {Card, Col, Container, Row, Form, Button, OverlayTrigger, Tooltip, Modal, Alert} from 'react-bootstrap'
...
class DetailVacancy extends React.Component {
    ...
    componentDidMount()
    {
         ...
         axios.get('/get_all_vacancy_info/'+id).then(res=>{
           ...
           this.setState({deadline:res.data[0].days});
           this.setState({experience:res.data[0].vacancy_experience});
           this.setState({position:res.data[0].vacancy_position});
           this.setState({vacancyType:res.data[0].vacancy_type});
           this.setState({EmployeeType:res.data[0].vacancy_employee_type});
        ...
        try {
            axios.post('/save_apply',data).then(res=>{
              if(res.data[0].applied_vacancy_auto){
                ...
                this.handleResponseToast(`Thank you! This is the Confirmation ID,<span style="color:green">${res.data[0].applied_vacancy_auto}</span>. Don't forget this ID!`,true);
        ...
        return (
          ...
          <Container>
             <br />
               ...
               <Col sm={4}><b>Position: {this.state.position}</b> </Col>
                  <Col sm={2}>| EXP: {this.state.experience}  </Col>
                  <Col sm={2}>| {this.state.EmployeeType} </Col>
                  <Col sm={2}>| {this.state.vacancyType}</Col>
                  <Col sm={2} style={{color:"red"}}>| {this.state.deadline>=0?this.state.deadline+' days reamin':'Closed'}</Col>
                  <hr />
                <Col sm={12}>
                    {this.state.rowsData}
                </Col>
