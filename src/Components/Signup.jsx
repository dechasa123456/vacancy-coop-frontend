...
import { Col, Row, Tab, Nav, Container, Card, Form, Button, Modal, Table, Toast, ToastContainer } from 'react-bootstrap';
...
//  Data fetching for all tabs end
  ...
  //  console.log(this.file)
      ...
      try {
        ...
        axios.post('/add_certificate',data).then(res=>{
...
<Tab.Container id="left-tabs-example" defaultActiveKey="first">
  <Row>
    <Col sm={3}>
      ...
      <Nav variant="pills" className="flex-column">
        ...
        <Nav.Item>
          <Nav.Link eventKey="my-applied" onClick={this.myApplied}>My Applied</Nav.Link>
        </Nav.Item>
      </Nav>
      </Card>
  ...
  {this.state.certificateRow.map((row,i)=>(
    <tr>
      ...
      <td><a href="" target="_blank"t><FaFileAlt /></a></td>
  ...
  <tbody>
    {this.state.myAppliedRow.map((row,i)=>(
      ...
      {/* model for references*/}
        ...
        <Form.Control onChange={(e=>this.phone=e.target.value)} />
        </Form.Group>
        </Col>
        </Row>
        <Row>
        ...
        <Form.Control onChange={(e=>this.relation=e.target.value)} />
        </Form.Group>
        </Col>
        <Button variant="info" style={{color:"#fff"}} type="submit">Save</Button>
        </Row>
